...
#define VM_MAX  593  // VM[V/10] max +10%
...
static inline TMC2590TypeDef *motorToIC(uint8 motor)
{
	UNUSED(motor);
...
// SPI Wrapper for API
void tmc2590_readWriteArray(uint8 channel, uint8 *data, size_t length)
{
...
//
//// => SPI wrapper
//void tmc2590_writeInt(uint8 motor, uint8 address, int value)
//{
//	UNUSED(motor);
//
...
//}
//
//uint32 tmc2590_readInt(uint8 motor, uint8 address)
//{
//	UNUSED(motor);
//
...
//}
//
//void tmc2590_readWrite(uint8 motor, uint32 value)
//{
//	UNUSED(motor);
//
//	static uint8 rdsel = 0; // number of expected read response
...
static void periodicJob(uint32 tick)
{
	...
	uint8 currCoolStepState = (abs(StepDir_getActualVelocity(DEFAULT_MOTOR)) >= TMC2590.coolStepThreshold);
	if(currCoolStepState != lastCoolStepState)
	{
		uint8 value = (currCoolStepState)? TMC2590.coolStepActiveValue : TMC2590.coolStepInactiveValue;
