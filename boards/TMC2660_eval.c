...
{ // mark if current should be reduced in stand still if too high
    ...
    if (TMC2660_GET_STST(tmc2660_getStatusBits(DEFAULT_ICID)))
    {
...
static uint32_t handleParameter(uint8_t readWrite, uint8_t motor, uint8_t type, int32_t *value)
{
    ...
    case 6:
        ...
        else if (readWrite == WRITE)
        {
            ...
            {
                if (TMC2660_GET_STST(tmc2660_getStatusBits(DEFAULT_ICID)) == 0)
                    tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_CS_FIELD, TMC2660.runCurrentScale);
            }
            else
            {
    ...
    case 7:
        ...
        else if (readWrite == WRITE)
        {
            ...
            if (Evalboards.ch1.fullCover == NULL)
            {
                if (TMC2660_GET_STST(tmc2660_getStatusBits(DEFAULT_ICID)) == 1)
                    tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_CS_FIELD, TMC2660.standStillCurrentScale);
            }
            else
            {
                TMC2660.runCurrentScale = *value;
    ...
    case 140:
        ...
        if (readWrite == READ)
        {
            *value = 8 - tmc2660_fieldRead(DEFAULT_ICID, TMC2660_MRES_FIELD);
        }
        else if (readWrite == WRITE)
        {
            switch (*value)
            ...
            if (*value != -1)
            {
                tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_MRES_FIELD, *value);
            }
            else
            {
    ...
    case 160:
        ...
        if (readWrite == READ)
        {
            *value = tmc2660_fieldRead(DEFAULT_ICID, TMC2660_INTPOL_FIELD);
        }
        else if (readWrite == WRITE)
        {
            tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_INTPOL_FIELD, *value);
        }
        break;
    case 161:
        // Double Edge Steps
        if (readWrite == READ)
        {
            *value = tmc2660_fieldRead(DEFAULT_ICID, TMC2660_DEDGE_FIELD);
        }
        else if (readWrite == WRITE)
        {
            tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_DEDGE_FIELD, *value);
        }
        break;
    case 162:
        // Chopper blank time
        if (readWrite == READ)
        {
            *value = tmc2660_fieldRead(DEFAULT_ICID, TMC2660_TBL_FIELD);
        }
        else if (readWrite == WRITE)
        {
            tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_TBL_FIELD, *value);
        }
        break;
    case 163:
        // Constant TOff Mode
        if (readWrite == READ)
        {
            *value = tmc2660_fieldRead(DEFAULT_ICID, TMC2660_CHM_FIELD);
        }
        else if (readWrite == WRITE)
        {
            tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_CHM_FIELD, *value);
        }
        break;
    case 164:
        ...
        if (readWrite == READ)
        {
            *value = tmc2660_fieldRead(DEFAULT_ICID, TMC2660_HDEC_FIELD);
        }
        else if (readWrite == WRITE)
        {
            tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_HDEC_FIELD, *value);
        }
        break;
    case 165:
        ...
        if (readWrite == READ)
        {
            *value = tmc2660_fieldRead(DEFAULT_ICID, TMC2660_HEND_FIELD);
        }
        else if (readWrite == WRITE)
        {
            tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_HEND_FIELD, *value);
        }
        break;
    case 166:
        ...
        if (readWrite == READ)
        {
            *value = tmc2660_fieldRead(DEFAULT_ICID, TMC2660_HSTRT_FIELD);
        }
        else if (readWrite == WRITE)
        {
            tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_HSTRT_FIELD, *value);
        }
        break;
    case 167:
        // Chopper off time
        if (readWrite == READ)
        {
            *value = tmc2660_fieldRead(DEFAULT_ICID, TMC2660_TOFF_FIELD);
        }
        else if (readWrite == WRITE)
        {
            tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_TOFF_FIELD, *value);
        }
        break;
    case 168:
        ...
        if (readWrite == READ)
        {
            *value = tmc2660_fieldRead(DEFAULT_ICID, TMC2660_SEIMIN_FIELD);
        }
        else if (readWrite == WRITE)
        {
            tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_SEIMIN_FIELD, *value);
        }
        break;
    case 169:
        ...
        if (readWrite == READ)
        {
            *value = tmc2660_fieldRead(DEFAULT_ICID, TMC2660_SEDN_FIELD);
        }
        else if (readWrite == WRITE)
        {
            tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_SEDN_FIELD, *value);
        }
        break;
    case 170:
        ...
        if (readWrite == READ)
        {
            *value = tmc2660_fieldRead(DEFAULT_ICID, TMC2660_SEMAX_FIELD);
        }
        else if (readWrite == WRITE)
        {
            tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_SEMAX_FIELD, *value);
        }
        break;
    case 171:
        ...
        if (readWrite == READ)
        {
            *value = tmc2660_fieldRead(DEFAULT_ICID, TMC2660_SEUP_FIELD);
        }
        else if (readWrite == WRITE)
        {
            tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_SEUP_FIELD, *value);
        }
        break;
    case 172:
        ...
        if (readWrite == READ)
        {
            *value = tmc2660_fieldRead(DEFAULT_ICID, TMC2660_SEMIN_FIELD);
        }
        else if (readWrite == WRITE)
        {
            tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_SEMIN_FIELD, *value);
        }
        break;
    case 173:
        ...
        if (readWrite == READ)
        {
            *value = tmc2660_fieldRead(DEFAULT_ICID, TMC2660_SFILT_FIELD);
        }
        else if (readWrite == WRITE)
        {
            tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_SFILT_FIELD, *value);
        }
        break;
    case 174:
        ...
        if (readWrite == READ)
        {
            *value = tmc2660_fieldRead(DEFAULT_ICID, TMC2660_SGT_FIELD);
            *value = CAST_Sn_TO_S32(*value, 7);
        }
        else if (readWrite == WRITE)
        {
            tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_SGT_FIELD, *value);
        }
        break;
    case 175:
        ...
        if (readWrite == READ)
        {
            *value = tmc2660_fieldRead(DEFAULT_ICID, TMC2660_SLPH_FIELD);
        }
        else if (readWrite == WRITE)
        {
            tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_SLPH_FIELD, *value);
        }
        break;
    case 176:
        ...
        if (readWrite == READ)
        {
            *value = tmc2660_fieldRead(DEFAULT_ICID, TMC2660_SLPL_FIELD);
        }
        else if (readWrite == WRITE)
        {
            tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_SLPL_FIELD, *value);
        }
        break;
    case 177:
        ...
        if (readWrite == READ)
        {
            *value = tmc2660_fieldRead(DEFAULT_ICID, TMC2660_DISS2G_FIELD);
        }
        else if (readWrite == WRITE)
        {
            tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_DISS2G_FIELD, *value);
        }
        break;
    case 178:
        ...
        if (readWrite == READ)
        {
            *value = tmc2660_fieldRead(DEFAULT_ICID, TMC2660_TS2G_FIELD);
        }
        else if (readWrite == WRITE)
        {
            tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_TS2G_FIELD, *value);
        }
        break;
    case 179:
        // VSense
        if (readWrite == READ)
        {
            *value = tmc2660_fieldRead(DEFAULT_ICID, TMC2660_VSENSE_FIELD);
        }
        else if (readWrite == WRITE)
        {
            tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_VSENSE_FIELD, *value);
        }
        break;
    case 180:
        ...
        if (readWrite == READ)
        {
            *value = tmc2660_fieldRead(DEFAULT_ICID, TMC2660_SE_FIELD);
        }
        else if (readWrite == WRITE)
        {
            errors |= TMC_ERROR_TYPE;
    ...
    case 183:
        ...
        if (readWrite == READ)
        {
            *value = tmc2660_fieldRead(DEFAULT_ICID, TMC2660_SDOFF_FIELD);
        }
        else if (readWrite == WRITE)
        {
            tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_SDOFF_FIELD, *value);
        }
        break;
    case 184:
        // Random TOff mode
        if (readWrite == READ)
        {
            *value = tmc2660_fieldRead(DEFAULT_ICID, TMC2660_RNDTF_FIELD);
        }
        else if (readWrite == WRITE)
        {
            tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_RNDTF_FIELD, *value);
        }
        break;
    case 185:
        ...
        if (readWrite == READ)
        {
            *value = tmc2660_fieldRead(DEFAULT_ICID, TMC2660_TST_FIELD);
        }
        else if (readWrite == WRITE)
        {
            tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_TST_FIELD, *value);
        }
        break;
    case 206:
        // Load value
        if (readWrite == READ)
        {
            *value = (compatibilityMode) ? tmc2660_fieldRead(DEFAULT_ICID, TMC2660_SGU_FIELD) << 5 :
                                           tmc2660_fieldRead(DEFAULT_ICID, TMC2660_SG2_FIELD);
        }
        else if (readWrite == WRITE)
        {
            errors |= TMC_ERROR_TYPE;
    ...
    case 208:
        ...
        if (readWrite == READ)
        {
            *value = tmc2660_getStatusBits(DEFAULT_ICID);
        }
        else if (readWrite == WRITE)
        {
            errors |= TMC_ERROR_TYPE;
...
{
    UNUSED(motor);
    tmc2660_writeRegister(DEFAULT_ICID, (uint8_t) address, value);
}

static void readRegister(uint8_t motor, uint16_t address, int32_t *value)
{
    UNUSED(motor);
    *value = tmc2660_readRegister(DEFAULT_ICID, (uint8_t) address);
}
...
static void on_standstill_changed(uint8_t newStandstill)
{
    ...
    {
        TMC2660.runCurrentScale = tmc2660_fieldRead(DEFAULT_ICID, TMC2660_CS_FIELD);
        tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_CS_FIELD, TMC2660.standStillCurrentScale);
    }
    else if (newStandstill == false)
    {
        TMC2660.standStillCurrentScale = tmc2660_fieldRead(DEFAULT_ICID, TMC2660_CS_FIELD);
        tmc2660_fieldWrite(DEFAULT_ICID, TMC2660_CS_FIELD, TMC2660.runCurrentScale);
    }
}
...
static void periodicJob(uint32_t tick)
{
    ...
    if (Evalboards.ch1.fullCover == NULL)
    { // Standstill detection only when not using an additional motion controller
        // Apply current settings
        if ((stst = TMC2660_GET_STST(tmc2660_getStatusBits(DEFAULT_ICID))) != lastStandstillState)
        {
    ...
    Evalboards.ch2.errors = (TMC2660.isStandStillOverCurrent) ? (Evalboards.ch2.errors | ERRORS_I_STS) :
                                                                (Evalboards.ch2.errors & ~ERRORS_I_STS);
    Evalboards.ch2.errors = (TMC2660.isStandStillCurrentLimit) ? (Evalboards.ch2.errors | ERRORS_I_TIMEOUT_STS) :
                                                                 (Evalboards.ch2.errors & ~ERRORS_I_TIMEOUT_STS);

    uint8_t currCoolStepState = (abs(StepDir_getActualVelocity(DEFAULT_ICID)) >= TMC2660.coolStepThreshold);
    ...
    StepDir_periodicJob(DEFAULT_ICID);
}
