...
static inline TMC4331TypeDef *motorToIC(uint8 motor)
{
	UNUSED(motor);
...
// => SPI Wrapper
void tmc4331_readWriteArray(uint8 channel, uint8 *data, size_t length)
{
...
static void tmc4331_fullCover(uint8 *data, size_t length)
{
	tmc4331_readWriteCover(&TMC4331, data, length);
}

// The cover function emulates the SPI readWrite function
static uint8 tmc4331_cover(uint8 data, uint8 lastTransfer)
{
	...
	if(lastTransfer)
	{
		...
		coverIn = 0;
		if(coverLength > 4)
			coverIn |= (uint64) tmc4331_readInt(&TMC4331, TMC4331_COVER_DRV_HIGH_RD) << 32;
