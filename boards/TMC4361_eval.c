...
static uint32_t handleParameter(uint8_t readWrite, uint8_t motor, uint8_t type, int32_t *value)
{
	...
	case 14:
		// Ramp type
		if(readWrite == READ) {
			uint32_t rampmode = TMC4361_FIELD_READ(motorToIC(motor), TMC4361_RAMPMODE, TMC4361_RAMP_PROFILE_MASK, TMC4361_RAMP_PROFILE_SHIFT);
			*value = (rampmode == TMC4361_RAMP_SSHAPE);
		} else if(readWrite == WRITE) {
			uint32_t rampmode = (*value) ? TMC4361_RAMP_SSHAPE : TMC4361_RAMP_TRAPEZ;
			TMC4361_FIELD_WRITE(motorToIC(motor), TMC4361_RAMPMODE, TMC4361_RAMP_PROFILE_MASK, TMC4361_RAMP_PROFILE_SHIFT, rampmode);
		}
		break;
	case 15:
		// Velocity VSTART
	...
	case 117:
		...
		if(readWrite == READ) {
			*value = TMC4361.config->shadowRegister[TMC4361_PID_I_WR] >> 0;
			*value &= 0x7FFF;
		} else if(readWrite == WRITE) {
			uvalue = TMC4361.config->shadowRegister[TMC4361_PID_I_WR];
			uvalue &= ~(0x7FFF << 0);
	...
	case 118:
		...
		// (JE) probably register TMC4361_PID_I_CLIP_WR?
		if(readWrite == READ) {
			*value = TMC4361.config->shadowRegister[TMC4361_PID_I_WR] >> 16;
			*value &= 0xFF;
		} else if(readWrite == WRITE) {
			uvalue = TMC4361.config->shadowRegister[TMC4361_PID_I_WR];
			uvalue &= ~(0xFF << 16);
	...
	case 136:
		...
		if(readWrite == READ) {
			*value = (TMC4361.config->shadowRegister[TMC4361_ENC_VMEAN_WAIT_WR] >> 0) &  0xFF;
			...
			uvalue = TMC4361.config->shadowRegister[TMC4361_ENC_VMEAN_WAIT_WR];
			uvalue &= ~(0xFF << 0);
			uvalue |= (*value & 0x0F) << 0;
...
static void writeRegister(uint8_t motor, uint8_t address, int32_t value)
{
	...
	case TMC4361_SCALE_VALUES:
		...
		case ID_TMC2130:
			TMC2130_FIELD_WRITE((TMC2130TypeDef *)Evalboards.ch2.type, TMC2130_IHOLD_IRUN, TMC2130_IHOLD_MASK, TMC2130_IHOLD_SHIFT, FIELD_GET(value, TMC4361_HOLD_SCALE_VAL_MASK, TMC4361_HOLD_SCALE_VAL_SHIFT));
			break;
		case ID_TMC2160:
			TMC2160_FIELD_WRITE((TMC2160TypeDef *)Evalboards.ch2.type, TMC2160_IHOLD_IRUN, TMC2160_IHOLD_MASK, TMC2160_IHOLD_SHIFT, FIELD_GET(value, TMC4361_HOLD_SCALE_VAL_MASK, TMC4361_HOLD_SCALE_VAL_SHIFT));
			break;
		}
		break;
	}
