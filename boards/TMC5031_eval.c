...
static uint32_t handleParameter(uint8_t readWrite, uint8_t motor, uint8_t type, int32_t *value)
{
    ...
    case 6:
        // Maximum current
        if(readWrite == READ) {
            *value = tmc5031_fieldRead(DEFAULT_ICID, TMC5031_IRUN_FIELD(motor));
        } else if(readWrite == WRITE) {
            tmc5031_fieldWrite(DEFAULT_ICID, TMC5031_IRUN_FIELD(motor), *value);
        }
        break;
    case 7:
        // Standby current
        if(readWrite == READ) {
            *value = tmc5031_fieldRead(DEFAULT_ICID, TMC5031_IHOLD_FIELD(motor));
        } else if(readWrite == WRITE) {
            tmc5031_fieldWrite(DEFAULT_ICID, TMC5031_IHOLD_FIELD(motor), *value);
        }
        break;
    case 8:
        // Position reached flag
        if(readWrite == READ) {
            *value = tmc5031_fieldRead(DEFAULT_ICID, TMC5031_POSITION_REACHED_FIELD(motor));
        } else if(readWrite == WRITE) {
            errors |= TMC_ERROR_TYPE;
        }
        break;
    case 10:
        // Right endstop
        if(readWrite == READ) {
            *value = !tmc5031_fieldRead(DEFAULT_ICID,TMC5031_STATUS_STOP_R_FIELD(motor));
        } else if(readWrite == WRITE) {
            errors |= TMC_ERROR_TYPE;
        }
        break;
    case 11:
        // Left endstop
        if(readWrite == READ) {
            *value = !tmc5031_fieldRead(DEFAULT_ICID, TMC5031_STATUS_STOP_L_FIELD(motor));
        } else if(readWrite == WRITE) {
            errors |= TMC_ERROR_TYPE;
        }
        break;
    case 12:
        ...
        if(readWrite == READ) {
            *value = tmc5031_fieldRead(DEFAULT_ICID, TMC5031_STOP_R_ENABLE_FIELD(motor));
        } else if(readWrite == WRITE) {
            tmc5031_fieldWrite(DEFAULT_ICID,TMC5031_STOP_R_ENABLE_FIELD(motor), !*value);
        }
        break;
    case 13:
        // Automatic left stop
        if(readWrite == READ) {
            *value = tmc5031_fieldRead(DEFAULT_ICID, TMC5031_STOP_L_ENABLE_FIELD(motor));
        } else if(readWrite == WRITE) {
            tmc5031_fieldWrite(DEFAULT_ICID,  TMC5031_STOP_L_ENABLE_FIELD(motor), (*value)? 0:1);    // todo CHECK 3: Ist 0:1 sorum richtig? Der alte code hatte das so (LH)
        }
        break;
    case 14:
        // SW_MODE Register
    ...
    case 28:
        ...
        if(readWrite == READ) {
            *value = tmc5031_fieldRead(DEFAULT_ICID, TMC5031_VHIGHFS_FIELD(motor));
        } else if(readWrite == WRITE) {
            tmc5031_fieldWrite(DEFAULT_ICID, TMC5031_VHIGHFS_FIELD(motor), *value);
        }
        break;
    case 29:
        if(readWrite == READ) {
    ...
    case 140:
        ...
        if(readWrite == READ) {
            *value = 256 >> tmc5031_fieldRead(DEFAULT_ICID, TMC5031_MRES_FIELD(motor));
        } else if(readWrite == WRITE) {
            switch(*value)
            {
            ...
            if(*value != -1)
            {
                ...
                tmc5031_fieldWrite(DEFAULT_ICID, TMC5031_MRES_FIELD(motor), *value);
            }
    ...
    case 162:
        // Chopper blank time
        if(readWrite == READ) {
            *value = tmc5031_fieldRead(DEFAULT_ICID, TMC5031_TBL_FIELD(motor));
        } else if(readWrite == WRITE) {
            tmc5031_fieldWrite(DEFAULT_ICID, TMC5031_TBL_FIELD(motor), *value);
        }
        break;
    case 163:
        // Constant TOff Mode
        if(readWrite == READ) {
            *value = tmc5031_fieldRead(DEFAULT_ICID, TMC5031_CHM_FIELD(motor));
        } else if(readWrite == WRITE) {
            tmc5031_fieldWrite(DEFAULT_ICID, TMC5031_CHM_FIELD(motor), *value);
        }
        break;
    case 164:
        // Disable fast decay comparator
        if(readWrite == READ) {
            *value = tmc5031_fieldRead(DEFAULT_ICID, TMC5031_DISFDCC_FIELD(motor));
        } else if(readWrite == WRITE) {
            tmc5031_fieldWrite(DEFAULT_ICID, TMC5031_DISFDCC_FIELD(motor), *value);
        }
        break;
    case 165:    // todo CHECK AP 2: Is there a particular reason why HEND and TFD are grouped to one axis parameter and sine offset and HSTART to another, even though HEND and sine offset share a bitfield, and HSTART and TFD another?
        ...
        if(readWrite == READ) {
            ...
            {
                *value = tmc5031_fieldRead(DEFAULT_ICID, TMC5031_HEND_FIELD(motor));
            }
            else // fast decay time
            {
                *value = tmc5031_fieldRead(DEFAULT_ICID, TMC5031_TFD_2__0__FIELD(motor));
            ...
            if(tempValue & TMC5031_CHM_MASK) // Chopper Hysteresis
            {
                tmc5031_fieldWrite(DEFAULT_ICID, TMC5031_HEND_FIELD(motor), *value);
            }
            else // fast decay time
            {
                ...
                tmc5031_fieldWrite(DEFAULT_ICID, TMC5031_TFD_2__0__FIELD(motor), *value);
            }
        }
        break;
    case 166:
        ...
        if(readWrite == READ) {
            ...
            {
                *value = tmc5031_fieldRead(DEFAULT_ICID, TMC5031_HSTRT_FIELD(motor));
            }
            else // sine wave offset
            {
                *value = tmc5031_fieldRead(DEFAULT_ICID, TMC5031_OFFSET_FIELD(motor));
            }
        } else if(readWrite == WRITE) {
            if(tempValue & TMC5031_CHM_MASK) // Chopper hysteresis start
            {
                tmc5031_fieldWrite(DEFAULT_ICID, TMC5031_HSTRT_FIELD(motor), *value);
            }
            else // sine wave offset
            {
                tmc5031_fieldWrite(DEFAULT_ICID, TMC5031_OFFSET_FIELD(motor), *value);
            }
        }
        break;
    case 167:
        // Chopper off time
        if(readWrite == READ) {
            *value = tmc5031_fieldRead(DEFAULT_ICID, TMC5031_TOFF_FIELD(motor));
        } else if(readWrite == WRITE) {
            tmc5031_fieldWrite(DEFAULT_ICID, TMC5031_TOFF_FIELD(motor), *value);
        }
        break;
    case 168:
        // smartEnergy current minimum (SEIMIN)
        if(readWrite == READ) {
            *value = tmc5031_fieldRead(DEFAULT_ICID, TMC5031_SEIMIN_FIELD(motor));
        } else if(readWrite == WRITE) {
            tmc5031_fieldWrite(DEFAULT_ICID, TMC5031_SEIMIN_FIELD(motor), *value);
        }
        break;
    case 169:
        // smartEnergy current down step
        if(readWrite == READ) {
            *value = tmc5031_fieldRead(DEFAULT_ICID, TMC5031_SEDN_FIELD(motor));
        } else if(readWrite == WRITE) {
            tmc5031_fieldWrite(DEFAULT_ICID, TMC5031_SEDN_FIELD(motor), *value);
        }
        break;
    case 170:
        // smartEnergy hysteresis
        if(readWrite == READ) {
            *value = tmc5031_fieldRead(DEFAULT_ICID, TMC5031_SEMAX_FIELD(motor));
        } else if(readWrite == WRITE) {
            tmc5031_fieldWrite(DEFAULT_ICID, TMC5031_SEMAX_FIELD(motor), *value);
        }
        break;
    case 171:
        // smartEnergy current up step
        if(readWrite == READ) {
            *value = tmc5031_fieldRead(DEFAULT_ICID, TMC5031_SEUP_FIELD(motor));
        } else if(readWrite == WRITE) {
            tmc5031_fieldWrite(DEFAULT_ICID, TMC5031_SEUP_FIELD(motor), *value);
        }
        break;
    case 172:
        // smartEnergy hysteresis start
        if(readWrite == READ) {
            *value = tmc5031_fieldRead(DEFAULT_ICID,  TMC5031_SEMIN_FIELD(motor));
        } else if(readWrite == WRITE) {
            tmc5031_fieldWrite(DEFAULT_ICID, TMC5031_SEMIN_FIELD(motor), *value);
        }
        break;
    case 173:
        // stallGuard2 filter enable
        if(readWrite == READ) {
            *value = tmc5031_fieldRead(DEFAULT_ICID, TMC5031_SFILT_FIELD(motor));
        } else if(readWrite == WRITE) {
            tmc5031_fieldWrite(DEFAULT_ICID, TMC5031_SFILT_FIELD(motor), *value);
        }
        break;
    case 174:
        // stallGuard2 threshold
        if(readWrite == READ) {
            *value = tmc5031_fieldRead(DEFAULT_ICID, TMC5031_SGT_FIELD(motor));
            *value = CAST_Sn_TO_S32(*value, 7);
        } else if(readWrite == WRITE) {
            tmc5031_fieldWrite(DEFAULT_ICID, TMC5031_SGT_FIELD(motor), *value);
        }
        break;
    case 179:
        // VSense
        if(readWrite == READ) {
            *value = tmc5031_fieldRead(DEFAULT_ICID, TMC5031_VSENSE_FIELD(motor));
        } else if(readWrite == WRITE) {
            tmc5031_fieldWrite(DEFAULT_ICID,  TMC5031_VSENSE_FIELD(motor), *value);
        }
        break;
    case 180:
        // smartEnergy actual current
        if(readWrite == READ) {
            *value = tmc5031_fieldRead(DEFAULT_ICID,  TMC5031_CS_ACTUAL_FIELD(motor));
        } else if(readWrite == WRITE) {
            errors |= TMC_ERROR_TYPE;
        }
        break;
    case 181:
        ...
        if(readWrite == READ)
        {
            ...
            tmc5031_fieldWrite(DEFAULT_ICID, TMC5031_SG_STOP_FIELD(motor), (*value) ? 1:0);
        }
        break;
    case 182:
        // smartEnergy threshold speed
    ...
    case 184:
        // Random TOff mode
        if(readWrite == READ) {
            *value = tmc5031_fieldRead(DEFAULT_ICID, TMC5031_RNDTF_FIELD(motor));
        } else if(readWrite == WRITE) {
            tmc5031_fieldWrite(DEFAULT_ICID, TMC5031_RNDTF_FIELD(motor), *value);
        }
        break;
    case 185:
        // Chopper synchronization
        if(readWrite == READ) {
            *value = tmc5031_fieldRead(DEFAULT_ICID, TMC5031_SYNC_FIELD(motor));
        } else if(readWrite == WRITE) {
            tmc5031_fieldWrite(DEFAULT_ICID,  TMC5031_SYNC_FIELD(motor), *value);
        }
        break;
    case 206:
        // Load value
        if(readWrite == READ) {
            *value = tmc5031_fieldRead(DEFAULT_ICID,  TMC5031_SG_RESULT_FIELD(motor));
        } else if(readWrite == WRITE) {
            errors |= TMC_ERROR_TYPE;
        }
        break;
