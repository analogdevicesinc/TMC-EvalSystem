...
static uint32_t handleParameter(uint8_t readWrite, uint8_t motor, uint8_t type, int32_t *value)
{
	...
	case 214:
		...
		if(readWrite == READ) {
			int adc = TMC5271_FIELD_READ(motorToIC(motor), TMC5271_IOIN, TMC5271_ADC_TEMPERATURE_MASK, TMC5271_ADC_TEMPERATURE_SHIFT);
...
static void writeRegister(uint8_t motor, uint8_t address, int32_t value)
{
	...
	if (address == 0x31 && value == 2) {
		tmc5271_writeInt(motorToIC(motor), address, 3);
		return;
	}
...
static void periodicJob(uint32_t tick)
{
	...
	for(int motor = 0; motor < TMC5271_MOTORS; motor++)
		{
