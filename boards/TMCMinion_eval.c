#include "Board.h"
#include "tmc/ic/TMCMinion/TMCMinion.h"
#include "../tmc/StepDir.h"

#undef  TMCMinion_MAX_VELOCITY
#define TMCMinion_MAX_VELOCITY  STEPDIR_MAX_VELOCITY
...
static RXTXTypeDef *TMCMinion_UARTChannel;
static TMCMinionTypeDef TMCMinion;
static ConfigurationTypeDef *TMCMinion_config;

// Helper macro - index is always 1 here (channel 1 <-> index 0, channel 2 <-> index 1)
#define TMCMinion_CRC(data, length) tmc_CRC8(data, length, 1)
...
void tmc_minion_writeRegister(u8 motor, uint8 address, int32 value)
{
	UNUSED(motor);
	UART_writeInt(TMCMinion_UARTChannel, tmc_minion_get_slave(&TMCMinion), address, value);
}

void tmc_minion_readRegister(u8 motor, uint8 address, int32 *value)
{
	UNUSED(motor);
	UART_readInt(TMCMinion_UARTChannel, tmc_minion_get_slave(&TMCMinion), address, value);
}
...
static uint32 handleParameter(u8 readWrite, u8 motor, u8 type, int32 *value)
{
	...
	case 6:
		// UART slave address
		if(readWrite == READ) {
			*value = tmc_minion_get_slave(&TMCMinion);
		} else if(readWrite == WRITE) {
			tmc_minion_set_slave(&TMCMinion, *value);
		}
		break;
	default:
		errors |= TMC_ERROR_TYPE;
...
static uint8 reset()
{
	...
	return tmc_minion_reset(TMCMinion_config);
}

static uint8 restore()
{
	return tmc_minion_restore(TMCMinion_config);
}
...
static void periodicJob(uint32 tick)
{
	...
	{
		tmc_minion_periodicJob(motor, tick, &TMCMinion, TMCMinion_config);
		StepDir_periodicJob(motor);
	}
}

void TMCMinion_init(void)
{
	tmc_fillCRC8Table(0x07, TRUE, 1);

	tmc_minion_initConfig(&TMCMinion);
	...
	TMCMinion_UARTChannel = HAL.UART;

	TMCMinion_config = Evalboards.ch2.config;
	...
	Evalboards.ch2.writeRegister        = tmc_minion_writeRegister;
	Evalboards.ch2.readRegister         = tmc_minion_readRegister;
