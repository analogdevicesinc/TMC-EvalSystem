...
/*
 * As the rising edge of Motion-Controller (MC) and Driver appear almost at the same time, there
 * is a possibility of missing edge of either one of them. So, as a workaround we make a variable
 * "isMC" and set it as false initially. It will be only set to true if MC is detected.
 * case 1: When both MC and Driver are connected, MC id is read automatically under PC7_8_IRQHandler
 * and for the driver we have to manually select from the IDE.
 * case 2: If only driver is connected, it will be detected automatically
 */
